...
def _get_cpu_cell() -> dict:
    ...
    return {"icon": CPU_ICON, "icon_bg_color": FOREGROUND_COL, "text": f"{int(pct)}%"}


def _get_gpu_cell() -> dict:
    # use nvidia-smi for gpu load
    try:
        ...
        pct = 0
    return {"icon": GPU_ICON, "icon_bg_color": FOREGROUND_COL, "text": f"{pct}%"}


def _create_cells() -> list[dict]:
    return [
        _get_active_process_cell(),
        _get_cpu_cell(),
        _get_gpu_cell(),
    ]
...
def _draw_right_status(screen: Screen, draw_data: DrawData) -> None:
    ...
    cells = _create_cells()
    total = sum(len(LEFT_ROUND) + len(c['icon']) + len(c['text']) + len(RIGHT_ROUND) + 1 for c in cells)
